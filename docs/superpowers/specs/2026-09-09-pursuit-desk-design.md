...
Persistence: in `toUIMessageStreamResponse({ originalMessages, onEnd })`, when `!isAborted` and `outcome.status === "completed"` and the final text is non-empty, store the user message and the answer in one `db.batch`, with `sources` collected from the response message's tool parts (URLs from `search_web` output, document titles from `read_document`). The route calls `result.consumeStream()` so the stream runs to completion and `onEnd` fires when the drawer is closed early; the answer then appears on reload. Nothing is stored on abort, failure or empty output. Each answer shows its sources beneath it. "Clear" deletes the thread after a confirm.
