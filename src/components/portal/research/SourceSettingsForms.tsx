...
export function SourceSettingsForms({ sources, reload }: {
  ...
  const taskButton = (target: View, text: string) => (
    <button type="button" disabled={pending} aria-pressed={view === target} onClick={() => choose(target)} className={`min-h-11 rounded px-4 py-2.5 text-[15px] font-medium disabled:opacity-50 ${view === target ? "bg-primary text-surface" : "border border-text/20 bg-white/50 hover:bg-white"}`}>{text}</button>
  );
    ...
    <div className="space-y-7">
      <div aria-label="Source tasks" className="flex flex-wrap items-start gap-2">
        ...
        <details className="group relative sm:ml-auto">
          <summary className={`min-h-11 cursor-pointer rounded px-4 py-2.5 text-[15px] ${view === "licences" || view === "documents" ? "bg-primary/10 font-medium" : "text-muted"}`}>Administration</summary>
          <div className="absolute right-0 z-10 mt-2 w-64 space-y-1 rounded border border-text/20 bg-surface p-2 shadow-lg">
            <button type="button" onClick={event => { choose("licences"); event.currentTarget.closest("details")?.removeAttribute("open"); }} className="block w-full rounded p-3 text-left text-[15px] hover:bg-primary/5">Licences and permissions</button>
            <button type="button" onClick={event => { choose("documents"); event.currentTarget.closest("details")?.removeAttribute("open"); }} className="block w-full rounded p-3 text-left text-[15px] hover:bg-primary/5">Document checks</button>
          </div>
        </details>
      </div>

      {refreshError && <p role="status" className="rounded border border-danger bg-[var(--danger-surface)] p-4 text-[15px]">{refreshError}</p>}
      ...
      {needsRecords && recordsError && <div role="alert" className="rounded border border-danger bg-[var(--danger-surface)] p-4"><p>{recordsError}</p><button type="button" className="mt-2 underline" onClick={() => setRevision(revision + 1)}>Try again</button></div>}
      <div hidden={view !== "licences"}>
        {opened.licences && rightsReady && <section className="space-y-5" aria-labelledby="source-licences-title">
          <div><h2 id="source-licences-title" className="font-sans text-2xl">Licences and permissions</h2><p className="mt-2 max-w-3xl text-[15px] text-muted">The permissions already recorded for your sources. Add a record only when using material covered by a new licence or set of terms.</p></div>
          {!rights.length && <p className="rounded border border-dashed border-text/25 p-5">No permissions have been recorded yet. Add the relevant licence or reuse terms below before registering a source.</p>}
          {rights.map(right => <details key={String(right.id)} className="rounded border border-text/15 bg-white/60 p-5">
            <summary className="cursor-pointer"><span className="font-medium text-primary">{String(right.holder)}</span><span className="mt-1 block text-[15px] text-muted">{String(right.material)}</span></summary>
            <dl className="mt-5 grid gap-4 text-[15px] sm:grid-cols-2">
              <div className="sm:col-span-2"><dt className="font-medium">Permitted use</dt><dd className="mt-1 text-muted">{String(right.purpose)}</dd></div>
              <div className="sm:col-span-2"><dt className="font-medium">Agreement or terms</dt><dd className="mt-1 break-words text-muted">{String(right.agreement_ref)}</dd></div>
          ...
          <details className="rounded border border-text/20 p-5">
        ...
        {view === "documents" && !recordsLoading && !recordsError && <section aria-labelledby="source-documents-title" className="space-y-5">
          <div><h2 id="source-documents-title" className="font-sans text-2xl">Document checks</h2><p className="mt-2 max-w-3xl text-[15px] text-muted">Review documents that are unavailable. Authorising a new check does not restore withdrawn text; the publisher must provide a fresh version first.</p></div>
          <p className="text-[15px] text-muted">Showing up to 200 recently updated document records. This is not a count of the full research collection.</p>
          <label className="flex items-center gap-2 text-[15px]"><input type="checkbox" checked={showAllDocuments} onChange={event => setShowAllDocuments(event.target.checked)} />Include available documents</label>
          {!documents.some(document => showAllDocuments || document.status !== "available") && <p className="rounded border border-line bg-white/60 p-5">No unavailable documents in the records shown.</p>}
          {documents.filter(document => showAllDocuments || document.status !== "available").map(document => <article key={String(document.id)} className="space-y-3 rounded border border-text/15 bg-white/60 p-5">
            <p className="font-medium">{String(document.source)}</p>
            <p className="break-all text-[15px] text-muted">{String(document.canonical_url || document.provider_id)}</p>
            <p className="text-[15px]">{String(document.status).replaceAll("_", " ")}{document.withdrawal_reason ? `: ${String(document.withdrawal_reason)}` : ""}</p>
            {document.status !== "available" && <details><summary className="cursor-pointer text-[15px] font-medium underline">Request a fresh publisher check</summary><div className="mt-4"><ActionForm title="Reason for a new check" button="Authorise fresh retrieval" fields={[{ ...field("reason", "Evidence and reason for checking again", "textarea"), hint: "Explain what has changed and give supporting evidence (at least 20 characters)." }]} submit={values => save(`documents/${document.id}/reinstate`, { reason: value(values, "reason") })} /></div></details>}
          </article>)}
        </section>}
    </div>
  );
