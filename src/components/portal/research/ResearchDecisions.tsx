  ...
  return <details className="rounded border border-text/15 p-4">
    ...
    <div className="space-y-4 pt-4">
      ...
      <ActionForm title="Record a decision" fields={[
        ...
        { name: 'ownerId', label: 'Assigned owner', type: 'select', options: directors.map(d => ({ value: String(d.id), label: String(d.name) })) },
      ...
      <p className="text-[13px] text-muted">Linking equivalent events removes duplicate corroboration and requires fresh review. Notes remain connected to the supporting source versions.</p>
      <DataTable rows={decisions} columns={[{ key: 'action', title: 'Decision' }, { key: 'note', title: 'Reason' }, { key: 'owner_id', title: 'Assigned owner', render: d => directors.find(x => x.id === d.owner_id)?.name as string ?? String(d.owner_id ?? 'Not assigned') }, { key: 'revisit_at', title: 'Revisit', render: d => date(d.revisit_at) }, { key: 'created_at', title: 'Recorded', render: d => date(d.created_at) }]} />
    </div>
  </details>;
}
