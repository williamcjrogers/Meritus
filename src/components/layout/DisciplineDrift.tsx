 ...
 * always visible, and a bright gold copy stacked exactly on top. The bright copy
 * is masked (in CSS) so it only shows through a band at the centre of the page —
 * so each keyword lights up gold as it passes the middle, then fades back.
...
export function DisciplineDrift() {
  ...
  return (
    ...
    >
      ...
      {/* Bright gold copy — revealed only where it crosses the centre of the page */}
      <div className="drift-spotlight pointer-events-none absolute inset-0 overflow-hidden">
        <div className="footer-drift whitespace-nowrap py-3 font-mono text-[9px] tracking-[0.3em] uppercase text-brass-light">
          {track}
        </div>
      </div>
    </div>
