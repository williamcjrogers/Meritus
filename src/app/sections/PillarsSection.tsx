...
export function PillarsSection() {
  ...
  useEffect(() => {
    ...
    const initialDelay = setTimeout(() => runSequence(0), 800);
                ...
                >
                  <div className={`pillar-card relative overflow-hidden h-full p-8 lg:p-10 border-t border-[#6da57e]/30 transition-all duration-[600ms] ease-out ${isActive ? "bg-[#112a1d]" : "bg-transparent"} ${index % 2 === 0 && index !== 4 ? "md:border-r md:border-r-[#6da57e]/30" : ""} ${index < 2 ? "md:border-t-0" : ""}`}>
                    ...
                    <div className="relative z-10 mt-4 flex items-center gap-2">
                      <span className={`font-mono text-[10px] tracking-[0.15em] uppercase text-brass transition-all duration-[600ms] ${isActive ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-2"}`}>
                        View detail
                      </span>
                      <svg width="12" height="12" viewBox="0 0 12 12" fill="none" className={`text-brass transition-all duration-[600ms] ${isActive ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-2"}`} aria-hidden="true">
