...
export default async function ActionsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [result, directory, initialAction] = await Promise.all([readActionViews(query, actorId, now).then(data => ({ ok: true as const, data })).catch(async error => { await recoverPageAccessError(error, "/portal/actions"); return { ok: false as const }; }), readDirectorDirectory(), editId.success ? readActionView(editId.data).catch(async error => { await recoverPageAccessError(error, "/portal/actions"); return null; }) : Promise.resolve(null)]);
